(function () {
  ...
  GF.createTerrain = () => {
    ...
    const terrain = new Array(WIDTH).fill(HEIGHT - 20);
    let y = 420 + Math.floor(Math.random() * 50);
    const step = 12;

    for (let x = 0; x < WIDTH; x += step) {
      y += (Math.random() - 0.5) * 24;
      y = GF.clamp(y, 290, 540);
      const nextX = Math.min(WIDTH - 1, x + step);
      const nextY = GF.clamp(y + (Math.random() - 0.5) * 8, 290, 540);
  ...
  GF.damageTerrain = (x, y, radius) => {
    ...
    for (let ix = start; ix <= end; ix++) {
      ...
      const carved = y + bowl;
      GF.state.terrain[ix] = GF.clamp(Math.max(GF.state.terrain[ix], carved), UI_HEIGHT + 10, HEIGHT - 2);
    }
  };
})();
